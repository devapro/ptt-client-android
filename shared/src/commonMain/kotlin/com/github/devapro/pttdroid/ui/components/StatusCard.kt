...
fun StatusCard(
    ...
    val title = when {
        status == PttUiStatus.RECEIVING && !holderName.isNullOrBlank() ->
            stringResource(Res.string.status_receiving_from, holderName)
    ...
    val subtitle = when (status) {
        ...
        else -> pluralStringResource(Res.plurals.peers_online, peers, peers)
    }
        ...
        Row(
            ...
            if (status == PttUiStatus.OFFLINE) {
                TextButton(onClick = onConnect) {
                    Text(stringResource(Res.string.main_connect))
                }
            } else {
                TextButton(onClick = onDisconnect) {
                    Text(stringResource(Res.string.main_disconnect))
                }
            }
        }
    }
