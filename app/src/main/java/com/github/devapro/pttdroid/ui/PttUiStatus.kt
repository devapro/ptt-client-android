 ...
 * Kept free of Compose and of Android so [of] can be unit-tested.
 */
enum class PttUiStatus(
    ...
    OFFLINE(0xFF64748B, R.string.status_disconnected, R.string.ptt_cap_offline),

    /** Socket in progress, including every automatic backoff retry. */
    CONNECTING(0xFFF59E0B, R.string.status_connecting, R.string.ptt_cap_linking),

    /** Connected and the floor is free — the only state in which pressing does anything. */
    READY(0xFF22C55E, R.string.status_ready, R.string.ptt_cap_talk),

    /** `talk_request` sent, server has not answered. Speaking now would clip the first word. */
    REQUESTING(0xFFF59E0B, R.string.status_requesting, R.string.ptt_cap_wait),

    /** The server granted us the floor; the microphone is open. */
    TRANSMITTING(0xFFEF4444, R.string.status_transmitting, R.string.ptt_cap_on_air),

    /** Somebody else holds the floor. Our button is inert until they release it. */
    RECEIVING(0xFF38BDF8, R.string.status_receiving, R.string.ptt_cap_busy),
    ;
