...
output:
  properties:
    ran:
      metadata:
        description: Commands run, one per line
      type: string
    passed:
      metadata:
        description: Tasks that passed, with counts/findings where relevant
      elements:
        type: string
    failed:
      metadata:
        description: Tasks that failed, with assertion/compiler message plus file and line
      elements:
        type: string
    not_run:
      metadata:
        description: Tasks skipped and why
      elements:
        type: string
---
...
- **Lint** — `app/build/reports/lint-results-debug.html` / `.txt` and the same under `shared/`.
  `:app` carries **12 pre-existing findings**; `:shared` carries **0**. Report the delta, not the
  total: a new `:app` finding or any `:shared` finding is the signal.
