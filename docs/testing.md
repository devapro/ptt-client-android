...
```bash
./gradlew :shared:testDebugUnitTest :shared:desktopTest   # 171 tests, on both targets — verified, 0 failures
...
ANDROID_SERIAL=<serial> ./gradlew :shared:connectedDebugAndroidTest   # 50 tests: 46 UI + 1 migration + 3 TLS (skipped without -Pandroid.testInstrumentationRunnerArguments.relayHost=...); ANDROID_SERIAL picks the device
```

The JVM counts below were re-verified for the start-of-talk tone (`:shared:testDebugUnitTest` and
`:shared:desktopTest` each produce 171 tests with 0 failures). The instrumented count is 50
(46 UI including the start-beep toggle, plus 1 migration and 3 TLS skipped without a relay) —
the extra UI test was not re-run on a device in this change; the toggle was checked on the
emulator via the installed debug APK instead.

## Unit tests (171, all passing, on both `androidTarget` and `desktop`)
...
| `audio/StartBeepTest` | 2 | The start-of-talk tone is three 40 ms wire frames, not silence, and ramps at the edges so it does not click |
| `data/settings/SettingsRepositoryTest` | 1 | The start-beep flag round-trips through a real DataStore; a missing key is on (fresh install) |
...
## Compose UI tests (46, all passing)
...
a real relay" below), which is where the full 50 in the command at the top of this page comes from.
