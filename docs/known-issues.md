...
| 36 | **A dead connection went on reporting "connected" until the user pressed talk.** A WebSocket that loses its network usually closes nothing and raises nothing — the read loop just never sees another frame — and neither side of the transport surfaces WebSocket ping/pong to application code, so nothing in the app could tell an idle link from a dead one. The relay noticed (it pings, and drops silent sessions); the client did not, and the first sign was a press that opened the microphone against a socket that was already gone | Two layers. OkHttp sends WebSocket pings every 15 s on Android and desktop (`createPttHttpClient`) and fails the socket when one goes unanswered, which arrives as an ordinary `Disconnected`; and `domain/KeepalivePolicy` + `PttController.runKeepalive` probe with a `ping` control message after one inbound-silent interval and redial after three, on every platform including iOS, whose engine has no ping of its own. See [`architecture.md`](architecture.md#noticing-that-the-socket-died) |
| 37 | **Settings' Save button sat underneath the navigation bar.** The app draws edge-to-edge (targetSdk 36 enforces it, see #13), and `Scaffold` pads its *body* for window insets but not its bars — a bar is expected to consume its own. `SettingsScreen`'s `bottomBar` applied only `imePadding()`, which is zero whenever the keyboard is closed, so the button rendered in the navigation bar's strip: partly covered with a gesture handle, entirely covered by a tablet's taskbar. Reproduced on a 2560x1600 tablet in 3-button mode, where the navigation bar occupies the bottom 120 px and the button's centre line landed at 1528 of 1600 | The `bottomBar` consumes `WindowInsets.safeDrawing.only(WindowInsetsSides.Bottom)`. That supersedes `imePadding()` rather than joining it: `safeDrawing` already includes the IME, so the button still lifts above an open keyboard, and the insets combine by maximum, so the keyboard case gains no extra navigation-bar padding. Zero on desktop, the home-indicator area on iOS. `MainScreen` was never affected — it has no `bottomBar`, so `Scaffold`'s own `contentWindowInsets` already covered it |
| 38 | **A failed playback setup was completely invisible.** `VoicePlayerContract.prepare()` returned `Unit`, so `VoicePlayer`'s four failure paths (an unsupported sample rate, an `AudioTrack.Builder` exception, a track that built but never reached `STATE_INITIALIZED`, a `play()` exception), `DesktopVoicePlayer`'s three, and any future platform's, all only logged — and on Android, only in a debug build (see #10/#39). The socket still connected, `welcome` still arrived, the UI still showed a normal "connected" state, and the user just heard nothing, with nothing anywhere distinguishing "the channel is quiet" from "audio is broken on this device". Five static investigation passes could not find the actual root cause of "no incoming audio on Android and desktop" for exactly this reason: the failure left no trace anywhere the state machine could see | `prepare()` now returns `Boolean` (`true` = ready to play) on every implementation (`VoicePlayer`, `DesktopVoicePlayer`, `IosVoicePlayer`, and the test fakes). `PttController`'s `Welcome` branch folds a `false` into `PttState.lastError` (`PLAYBACK_UNAVAILABLE`, next to `RELAY_SILENT`), the same way a dead keepalive already surfaces on the banner. A future playback failure now looks like every other reported error instead of like nothing at all |
| 39 | **A failed bip preamble stranded the transmission.** `PttController.startTransmit()` sent the two broadcast-start bip frames before starting the microphone, and returned out of the whole coroutine on the first one `connection.sendAudio` rejected — but the server had already granted the floor by then. The result was a talker holding the floor with the microphone never opened: every listener heard silence, and nothing on the talker's side showed an error, because as far as the rest of the state machine was concerned the transmission had simply never started | The bip is now best-effort: a failed frame stops sending the rest of the tone but always falls through to `recorder.start()` and the real audio loop. Coroutine cancellation (`!isActive`) still returns immediately — a bip failure is the only thing that was made non-fatal, not cancellation |
| 40 | **The on-device relay fanned out one shared `Frame.Binary` instance to every peer.** `InternalPttServer.relay()` (`internalserver/InternalPttServer.kt`) enqueued the SAME `Frame.Binary` onto every other session's outbound channel. Ktor's JVM `Frame` wraps its payload in a `buffer: ByteBuffer = ByteBuffer.wrap(data)` field created once per instance, carrying mutable read state (position/limit); with one writer coroutine per session, N peers' writers raced over that one shared `ByteBuffer` while serializing to their own sockets, so whichever write lost the race sent a truncated or empty payload. Only shows up at 3+ participants (2+ receiving peers), which is why 2-user channels and the existing 2-client tests always looked fine. The standalone `ptt-server` had the identical defect in `PttChannel.relayAudio`, fixed the same way there first | `relay()` now constructs a fresh `Frame.Binary(frame.fin, payload)` per peer, so each gets its own `ByteBuffer`; no array copy needed since Ktor hands `relay()` a freshly-allocated, unshared `ByteArray` per inbound frame (`WebSocketReader`/`SimpleFrameCollector`/`moveToByteArray()`, verified against `ktor` 3.5.2's sources), and reading through a `ByteBuffer` never mutates the backing array. Pinned by a 3-client test in `InternalPttServerTest` that sends several distinct-payload frames and asserts both other peers receive every frame byte-for-byte |
